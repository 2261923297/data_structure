...
#define _traverse(data_structure, visit) data_structure->bo->traverse(data_structure->st, visit, #visit)
...
#define test_func(func, ...)  do { \
		...
		printf("[test] %s:%d func [ %s ] is end\n", __FILE__, __LINE__, #func);						\
		printf("[test] %s:%d func [ %s ] using time is %" PRIu64 "us\n", __FILE__, __LINE__, #func, end - start);						\
	} while(0);												\
